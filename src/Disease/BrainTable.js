...
import { getBrainTable } from '../DataService/Service';
...
const handleData =async () =>
{
  let tok = localStorage.getItem("token")
   const res = await getBrainTable(tok)
   console.log(res);
   setResult(res);
   console.log(result[0])
//    if(res.length == 0){
...
return(
    <Grid container className="Medform">
       <NavMod title={"Brain Statistics"} n={birthName} />
...
<Grid item style={{width:"100%"}}>
    ...
    <Paper  style={{ height: "30em" ,margin:"5em 20em",width:"60%" }}>
        <Table striped bordered hover responsive="sm" variant="dark" size='md' style={{width:"100%"}}>
            <thead>
                <tr>
                <th colspan="2" style={{fontStyle:"italic" , fontSize:"1.5em"}}>Tumor Vs Result</th>
                </tr>
                <tr>
                <th>Type of Tumor</th>
                <th>Result</th>
